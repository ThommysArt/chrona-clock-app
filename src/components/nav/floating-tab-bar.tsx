...
const TAB_ICON = 22;
const TAB_LABEL = 10;
/** Compact pill — 3 tabs only, not full-bleed */
...
> = {
	index: { activeIcon: "list", icon: "list-outline", label: "List" },
	globe: { activeIcon: "globe", icon: "globe-outline", label: "Globe" },
	clock: { activeIcon: "time", icon: "time-outline", label: "Clock" },
};
...
/** Liquid-glass floating tab bar — sulphur structure, Chrona sizing/colors. */
export function FloatingTabBar({
	...
	const indicatorStyle = useAnimatedStyle(() => {
		...
		const itemWidth = Math.max(
			0,
			(itemsWidth.value - gap * (count - 1)) / count
		);
...
const styles = StyleSheet.create({
	...
	shell: {
		...
		shadowOpacity: 0.22,
		shadowRadius: 18,
		// Fixed compact width — 3 tabs only
		width: TAB_BAR_WIDTH,
	},
	shellDark: {
